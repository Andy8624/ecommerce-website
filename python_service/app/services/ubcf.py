...
class UserBasedCollaborativeFiltering:
    ...
    def build_user_item_matrix(self, interactions):
        ...
        weight_map = {
            ...
            "REMOVE_CART": -1,  # Trọng số cho hành động xóa khỏi giỏ hàng
            "PURCHASE": 3,      # Trọng số cho hành động mua hàng
            "RATING": 3,        # Trọng số cho hành động đánh giá sản phẩm
        }
    ...
    def integrate_reviews(self, user_item_matrix, reviews, scaling_factor=1.0):
        """
            ...
            Sử dụng mô hình 'cardiffnlp/twitter-roberta-base-sentiment-latest'
        ...
        for review in reviews:
            ...
            comment = review.get("comment", "")  # Lấy bình luận, mặc định là rỗng nếu không có
            ...
            if sentiment_label == "POS":
                ...
                sentiment_value = -sentiment_score
            # Nếu nhãn là "Neutral" có thể được ánh xạ về 0 hoặc một giá trị gần 0 (Noi chung la > 0)
    ...
    def recommend_by_user_based(self, user_id, user_item_matrix_df, csr_matrix, top_k=3):
        ...
        # Sử dụng NearestNeighbors để tìm top_k người dùng tương tự dựa trên độ tương đồng cosine
        ...
        # Lấy danh sách khoảng cách tương ứng
        ...
        for i, other_user_idx in enumerate(similar_users_indices):
            ...
            user_distance = user_distances[i + 1] # +1 vì bỏ qua chính mình ở indices[0][1:]
            # Chuyển khoảng cách thành độ tương đồng (cosine similarity = 1 - cosine distance)
            similarity = 1 - user_distance

            # Lấy vector tương tác của người dùng tương tự
            ...
            for item_idx, interaction_value in enumerate(other_user_interactions):
                if interaction_value > 0:  # Chỉ xem xét các sản phẩm có tương tác dương
